...
# ─────────────────────────────────────────────
    ...
    with crud_tab2:
        ...
        with st.form("add_form", clear_on_submit=True):
            ...
            with ac2:
                provider_type = st.selectbox("🏷️ Provider Type *",
            ...
            if submitted:
                if not food_name or not location:
                    ...
                    if ok:
                        ...
                        st.error("Failed to add food listing. Check Provider ID exists.")
        ...
        if df_ids.empty:
            ...
            if not df_current.empty:
                ...
                with st.form("update_form"):
                    ...
                    with uc2:
                        ptype_opts = ["Restaurant","Hotel","Supermarket","Household","Event","NGO","Other"]
                        u_prov_type = st.selectbox("🏷️ Provider Type", ptype_opts,
                                                   index=ptype_opts.index(row["Provider_Type"])
                                                   if row["Provider_Type"] in ptype_opts else 0)
                        u_location  = st.text_input("📍 Location", value=str(row["Location"]))
                        ftype_opts  = ["Veg","Non-Veg","Vegan","Jain","Gluten-Free"]
                        u_food_type = st.selectbox("🍲 Food Type", ftype_opts,
                                                   index=ftype_opts.index(row["Food_Type"])
                                                   if row["Food_Type"] in ftype_opts else 0)
                        mtype_opts  = ["Breakfast","Lunch","Dinner","Snack","Any"]
                        u_meal_type = st.selectbox("🕐 Meal Type", mtype_opts,
                                                   index=mtype_opts.index(row["Meal_Type"])
                                                   if row["Meal_Type"] in mtype_opts else 0)
...
<div class="app-footer">
    🌿 <b>Local Food Wastage Management System</b> &nbsp;|&nbsp;
    Built with Streamlit · MySQL · Pandas · Plotly &nbsp;|&nbsp;
    LABMENTIX Data Analytics Internship &nbsp;|&nbsp;
    © 2025 Amey · D.Y. Patil International University, Pune
</div>
""", unsafe_allow_html=True)
